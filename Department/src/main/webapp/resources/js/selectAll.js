...
// 왜? 이벤트리스너는 DOM 요소에 추가해야 동작하는데 
// deleteBtnList는 DOM 요소가 아닌 배열(유사배열, NodeList)
// ==> addEventListener 추가 못 함!

// 해결방법 -> 배열 내 각 인덱스 요소 == DOM 요소 (버튼)
...
for(let btn of deleteBtnList){ // 향상된 for문
  ...
  btn.addEventListener("click",e =>{
    ...
    //   CSS 선택자가 일치하는 요소를 찾을 때까지 검색
    //   일치하는 요소가 있으면 해당 요소 반환
    const deptId = e.target.closest("tr").children[1].innerText;

    console.log(deptId);

    if(confirm(`${deptId} 부서를 정말 삭제 하시겠습니다?`)){
      // 확인 클릭시

      // 삭제 요청 보내기 1)location.href --> 권장하진 않음
      // -> GET방식 요청
      // -> 브라우저 주소창에 입력하는 방법도 GET방식 요청
      //   => 임의의 사용자가 삭제요청을 마음대로 보내는 문제가 발생할 수 있음!!
      //location.href = "/department/delete?deptId=" + deptId;

      // 삭제 요청 보내기 2) form태그를 만들어서 POST방식 요청 보내기
      // 실무에서 많이 사용되는 방법 
